...
def get_timezone() -> str:
    """
    Finds the best match for timezone using the following priority:
    1. Timezone from URL parameters
    2. Timezone from user settings
    3. Default to UTC

    :return: The best matched timezone.
    """
...
def index() -> str:
    ...
    Return: The rendered 7-index.html template.
    """
    timezone = get_timezone()
    now = datetime.now(pytz.timezone(timezone))
    current_time = now.strftime("%b %d, %Y, %I:%M:%S %p")
    return render_template('index.html', home_title=_("home_title"), home_header=_("home_header"), current_time=_("current_time_is", current_time=current_time)), 200
